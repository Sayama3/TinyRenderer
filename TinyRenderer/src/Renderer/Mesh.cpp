...
namespace tr {
	...
	std::shared_ptr<Mesh> Primitives::CreateCube() {
		...
		std::vector<uint32_t> m_CubeIndices = {
				0, 3, 2, 0, 1, 3, // 0, 2, 3, 0, 3, 1,
				2, 7, 6, 2, 3, 7, // 2, 6, 7, 2, 7, 3,
				6, 5, 4, 6, 7, 5, // 6, 4, 5, 6, 5, 7,
				4, 1, 0, 4, 5, 1, // 4, 0, 1, 4, 1, 5,
				0, 6, 4, 0, 2, 6, // 0, 4, 6, 0, 6, 2,
				1, 7, 5, 1, 3, 7, // 1, 5, 7, 1, 7, 3,
		};

		return Mesh::Create(m_CubeVertices, m_CubeIndices);
	}

	std::shared_ptr<Mesh> Primitives::CreateCubemapMesh()
	{
		std::vector<Vertex> m_CubeVertices = {
				{{-1.0f, +1.0f, +1.0f}},
				{{-1.0f, -1.0f, +1.0f}},
